...
export default function ShawlPage() {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/products?category=shawl");
      ...
      if (res.ok) {
        ...
        setError("Failed to load shawl collection");
      }
    } catch (err) {
      setError("Network error - failed to load products");
      console.error("Error loading shawls:", err);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-300">Loading shawl collection...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-gradient-to-b from-black to-gray-900 py-20 px-6 border-b border-yellow-600/30">
        <div className="max-w-7xl mx-auto text-center">
          <h1 className="text-5xl md:text-6xl font-bold mb-4">
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-yellow-600">Shawl</span> Collection
          </h1>
          <p className="text-xl text-gray-400">Luxurious wraps for every occasion</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-20">
        ...
        {products.length === 0 && !loading && (
          <div className="text-center py-24">
            <p className="text-gray-400 text-xl mb-4">No shawls found in this collection</p>
            <a 
              href="/shopall" 
